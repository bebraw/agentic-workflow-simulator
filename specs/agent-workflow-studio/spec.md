...
- **Progressive disclosure:** Every stage should keep its primary controls and outputs visible by default while moving secondary teaching copy into contextual help toggles instead of permanent explanatory blocks.
...
- Secondary help text should stay available in every stage, but it should not crowd the primary task surface when the student is already working through examples, defining agents, building a workflow, or inspecting flow.
